...
```{r}
set.seed(44)
exprGroups = data.table(fracFeatures = c(0.75, 0.25), amp = c(0, 0.3))
simData = simphony(exprGroups, nFeatures = 200, interval = 2, nReps = 1, family = 'negbinom')
```

The output of `simphony` has three components: `exprData`, `sampleMetadata`, and `featureMetadata`. `exprData` is a matrix that contains the simulated expression values. Each row of corresponds to a gene, each column corresponds to a sample. Since we sampled from the negative binomial family, all expression values are integers.
```{r}
...
`featureMetadata` is a `data.table` that contains the properties of each simulated gene in each condition. The `group` column corresponds to the row in `exprGroups` to which the gene belongs.
```{r}
kable(simData$featureMetadata[149:151,]) %>%
...
```{r}
dExpect = getExpectedExpr(fmExample, 24, times = seq(0, 24, 0.25))
```

Then it all comes together with `ggplot`.
```{r, fig.width = 6, fig.height = 2.75}
dExample[, featureLabel := paste(feature, ifelse(amp == 0, '(non-rhythmic)', '(rhythmic)'))]
dExpect[, featureLabel := paste(feature, ifelse(amp == 0, '(non-rhythmic)', '(rhythmic)'))]
...
rhyLimma = merge(data.table(rhyLimma), simData$featureMetadata[, .(feature, amp)], by = 'feature')
```

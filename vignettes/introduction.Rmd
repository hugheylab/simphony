...
We will simulate a microarray experiment with a sample interval of 2 hours, with 1 replicate per time point.
```{r, message = FALSE}
set.seed(42)
...
We can see how our simulated expression samples compare to the expected range by plotting both a non-rhythmic and rhythmic gene. We also plot ribbons displaying the standard deviation of the Gaussian we sampled from.
...
We can measure `limma`'s performance by leveraging `simphony`'s ground-truth labels of rhythmicity to generate a Receiver-Operating Charactersitic curve.

...
Here we create a `data.table` called `abundGroups` that specifies the desired properties of the simulated genes. We want 75% of simulated genes to be non-rhythmic and 25% to have a rhythm amplitude of 1.1. Properties not specified in `abundGroups` will be given their default values.
...
```{r}
set.seed(44)
abundGroups = data.table(fracFeatures = c(0.75, 0.25), amp = c(0, 0.3))
simData = simphony(abundGroups, nFeatures = 200, interval = 2, nReps = 1, family = 'negbinom')
```
...
`featureMetadata` is a `data.table` that contains the properties of each simulated gene in each condition. The `group` column corresponds to the row in `abundGroups` to which the gene belongs.
```{r}
